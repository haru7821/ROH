...
> 3단계(§1 % 배율 어트리뷰트 7종 + % 접사/유니크·세트 % 옵션, §3.1 AP/SP 완전형 파이프라인 이관) — b29. 미적용 없음.
> 참고: 코드의 `Energy` 어트리뷰트가 본 문서의 INT 역할을 승계한다 (세이브 호환 유지, 명칭 이관은 추후).
> 예외(b29): %생명력 비례 피해(정전기장)는 AP/SP 배율 미적용 (`bApplyStatScaling=false`), 몬스터는 STR/INT 0으로 배율 1.0.
...
## 2. 생존/리소스 (2단계 b20 적용, % 배율은 3단계 b29)
...
### 3.1 공격력 (3단계 b29 완전 적용 — `UROHCombatStatics::ApplyDamage` 원피해 산정 단계)
